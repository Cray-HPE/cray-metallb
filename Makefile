...
dep-up:
	CMD="dep up charts/cray-metallb" $(MAKE) helm

lint:
	CMD="lint charts/cray-metallb" $(MAKE) helm
...
package:
ifdef CHART_VERSION
	CMD="package charts/cray-metallb --version $(CHART_VERSION) -d packages" $(MAKE) helm
else
...
images:
	CHART=charts/cray-metallb $(MAKE) -s extracted-images annotated-images | sort -u
...
clean:
	$(RM) -r .helm packages charts/cray-metallb/charts
